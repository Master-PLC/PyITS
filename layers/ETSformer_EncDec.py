...
class ExponentialSmoothing(nn.Module):
    ...
    def forward(self, values, aux_values=None):
        ...
        init_weight, weight = self.get_exponential_weight(t)  # [1, T, H, 1], [1, T, H, 1]
        output = conv1d_fft(self.dropout(values), weight, dim=1)  # [B, T, H, D]
        output = init_weight * self.v0 + output  # [B, T, H, D]
        ...
        return output  # [B, T, H, D]

    def get_exponential_weight(self, T):
        # Generate array [0, 1, ..., T-1]
        powers = torch.arange(T, dtype=torch.float, device=self.weight.device)  # [T]

        # (1 - \alpha) * \alpha^t, for all t = T-1, T-2, ..., 0]
        weight = (1 - self.weight) * (self.weight ** torch.flip(powers, dims=(0,)))  # [H, T]

        # \alpha^t for all t = 1, 2, ..., T
        init_weight = self.weight ** (powers + 1)  # [H, T]
...
class GrowthLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        b, t, d = inputs.shape
        values = self.in_proj(inputs).view(b, t, self.nhead, -1)  # [B, L, H, Dh]
        values = torch.cat([repeat(self.z0, 'h d -> b 1 h d', b=b), values], dim=1)  # [B, 1+L, H, Dh]
        values = values[:, 1:] - values[:, :-1]  # [B, L, H, Dh]
        out = self.es(values)  # [B, L, H, Dh]
        out = torch.cat([repeat(self.es.v0, '1 1 h d -> b 1 h d', b=b), out], dim=1)  # [B, 1+L, H, Dh]
        out = rearrange(out, 'b t h d -> b t (h d)')  # [B, L+1, H*Dh]
        return self.out_proj(out)  # [B, L+1, D]
...
class FourierLayer(nn.Module):
    ...
    def forward(self, x):
        """x: (b, t, d)"""
        b, t, d = x.shape
        x_freq = torch.fft.rfft(x, dim=1)  # [B, L//2 + 1, D]

        if t % 2 == 0:
            x_freq = x_freq[:, self.low_freq:-1]  # [B, L//2 - 1, D]
            f = torch.fft.rfftfreq(t, device=x_freq.device)[self.low_freq:-1]  # [L//2 - 1]
        else:
            x_freq = x_freq[:, self.low_freq:]
            f = torch.fft.rfftfreq(t, device=x_freq.device)[self.low_freq:]

        x_freq, index_tuple = self.topk_freq(x_freq)  # [B, topk, D], ([B, 1, D], [B, topk, D], [B, 1, D])
        f = repeat(f, 'f -> b f d', b=x_freq.size(0), d=x_freq.size(2))  # [B, L//2 - 1, D]
        f = rearrange(f[index_tuple], 'b f d -> b f () d')  # [B, topk, 1, D]
...
class LevelLayer(nn.Module):
    ...
    def forward(self, level, growth, season):
        b, t, _ = level.shape
        growth = self.growth_pred(growth)  # [B, T, Dx]
        season = self.season_pred(season)  # [B, T, Dx]
        growth = growth.view(b, t, self.c_out, 1)  # [B, T, Dx, 1]
        season = season.view(b, t, self.c_out, 1)  # [B, T, Dx, 1]
        level = level.view(b, t, self.c_out, 1)  # [B, T, Dx, 1]
        out = self.es(level - season, aux_values=growth)
        out = rearrange(out, 'b t h d -> b t (h d)')  # [B, T, Dx]
        return out
...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        self, d_model, nhead, c_out, seq_len, pred_len, k, dim_feedforward=None, dropout=0.1,
    ...
    def forward(self, res, level, attn_mask=None):
        # res: [B, L, d_model], level: [B, L, Dx]
        season = self._season_block(res)  # [B, L+P, D]
        res = res - season[:, :-self.pred_len]  # [B, L, D]
        growth = self._growth_block(res)  # [B, L+1, D]
        res = self.norm1(res - growth[:, 1:])  # [B, L, D]
        res = self.norm2(res + self.ff(res))  # [B, L, D]

        level = self.level_layer(level, growth[:, :-1], season[:, :-self.pred_len])  # [B, L, Dx]
        return res, level, growth, season  # [B, L, D], [B, L, Dx], [B, L+1, D], [B, L+P, D]
    ...
    def _season_block(self, x):
        x = self.seasonal_layer(x)  # [B, L+P, D]
        return self.dropout2(x)
...
class Encoder(nn.Module):
    ...
    def forward(self, res, level, attn_mask=None):
        ...
        for layer in self.layers:
            res, level, growth, season = layer(res, level, attn_mask=None)  # [B, L, D], [B, L, Dx], [B, L+1, D], [B, L+P, D]
