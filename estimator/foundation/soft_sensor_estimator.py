...
from utils.losses import WMSELoss
...
class Soft_Sensor_Estimator(Base_Estimator):
    ...
    def eval(self, x, y, criterion):
        ...
        for idx in range(eval_steps):
            ...
            batch_y = torch.tensor(y[batch_index], device=self.device)  # [B, Dy]

            # encoder - decoder
            if self.args.output_attention:
                outputs, attns = self.model(batch_x, None, None, None)  # [B, Dy]
            else:
                outputs = self.model(batch_x, None, None, None)  # [B, Dy]
        ...
        if len(x) % self.args.batch_size != 0:
            ...
            batch_y = torch.tensor(y[(idx + 1) * self.args.batch_size:], device=self.device)  # [B, Dy]
    ...
    def test(self, test=0):
        ...
        if len(x) % self.args.test_batch_size != 0:
            ...
            batch_y = torch.tensor(y[(idx + 1) * self.args.test_batch_size:], device=self.device)  # [B, Dy]
        ...
        message = f"Soft Sensor Test | mse: {self.metric_test['mse']:.7f}, r2: {self.metric_test['r2']:.7f}"
