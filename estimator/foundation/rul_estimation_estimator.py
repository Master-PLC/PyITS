...
from utils.losses import ConfidenceLoss
...
class RUL_Estimation_Estimator(Base_Estimator):
    ...
    def _select_criterion(self):
        ...
        elif loss == 'Confidence':
            criterion = ConfidenceLoss(self.args.num_seq, self.args.alpha)
        return criterion
    ...
    def eval(self, x, y, criterion):
        ...
        for idx in range(eval_steps):
            ...
            batch_y = torch.tensor(y[batch_index], device=self.device)  # [B, 1]

            # encoder - decoder
            if self.args.output_attention:
                outputs, attns = self.model(batch_x, None, None, None)  # [B, 1]
            else:
                outputs = self.model(batch_x, None, None, None)  # [B, 1]
        ...
        if len(x) % self.args.batch_size != 0:
            ...
            batch_y = torch.tensor(y[(idx + 1) * self.args.batch_size:], device=self.device)  # [B, 1]
    ...
    def fit(self):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            eval_loss = self.eval(x_eval, y_eval, nn.MSELoss())
    ...
    def test(self, test=0):
        ...
        if len(x) % self.args.test_batch_size != 0:
            batch_x = torch.tensor(x[(idx + 1) * self.args.test_batch_size:], device=self.device)  # [B, L, D]
            batch_y = torch.tensor(y[(idx + 1) * self.args.test_batch_size:], device=self.device)  # [B, 1]
        ...
        message = f"RUL Estimation Test | mse: {self.metric_test['mse']:.7f}, mae: {self.metric_test['mae']:.7f}"
