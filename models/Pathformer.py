...
from layers.Pathformer_EncDec import (MLP, FourierLayer, SparseDispatcher,
                                      Transformer_Layer, series_decomp_multi)
from layers.StandardNorm import Normalize


class AMS(nn.Module):
    def __init__(
        self, input_size, output_size, num_experts, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
        ...
        for patch in patch_size:
            ...
            self.experts.append(
                Transformer_Layer(
                    d_model=d_model, d_ff=d_ff, dynamic=dynamic, num_nodes=num_nodes, patch_nums=patch_nums,
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        expert_outputs = [
            self.experts[i](expert_inputs[i])[0] 
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.patch_size_list = np.array(configs.patch_size_list).reshape(self.layer_nums, -1).tolist()
        ...
        for num in range(self.layer_nums):
            self.AMS_lists.append(
                AMS(
                    self.seq_len, self.seq_len, self.num_experts_list[num], k=self.k, num_nodes=self.num_nodes, 
                    patch_size=self.patch_size_list[num], noisy_gating=True, d_model=self.d_model, d_ff=self.d_ff, 
                    layer_number=num + 1, residual_connection=self.residual_connection, batch_norm=self.batch_norm
                )
            )
    ...
    def forward(self, x, *args, **kwargs):
        ...
        # norm
        if self.revin:
            x = self.revin_layer(x, 'norm')  # [B, L, Dx]
        out = self.start_fc(x.unsqueeze(-1))  # [B, L, Dx, d_model]
        ...
        out = self.head(out).transpose(1, 2)
