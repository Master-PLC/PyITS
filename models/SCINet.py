...
class CausalConvBlock(nn.Module):
    ...
    def forward(self, x):
        # return value is the same as input dimension
...
class SCIBlock(nn.Module):
    ...
    def forward(self, x):
        x_even, x_odd = self.splitting(x)  # [B, L//2, Dx'], [B, L//2, Dx']
        x_even = x_even.permute(0, 2, 1)  # [B, Dx', L//2]
        x_odd = x_odd.permute(0, 2, 1)  # [B, Dx', L//2]

        x_even_temp = x_even.mul(torch.exp(self.modules_even(x_odd)))  # [B, Dx, L//2]
        x_odd_temp = x_odd.mul(torch.exp(self.modules_odd(x_even)))  # [B, Dx, L//2]

        x_even_update = x_even_temp + self.interactor_even(x_odd_temp)  # [B, Dx, L//2]
        x_odd_update = x_odd_temp - self.interactor_odd(x_even_temp)  # [B, Dx, L//2]

        return x_even_update.permute(0, 2, 1), x_odd_update.permute(0, 2, 1)  # [B, L//2, Dx], [B, L//2, Dx]
...
class SCINet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_even_update, x_odd_update = self.working_block(x)  # [B, L//2, Dx], [B, L//2, Dx]
        if odd_flag:
    ...
    def zip_up_the_pants(self, even, odd):
        # even: [B, L//2^(4-current_level), Dx], odd: [B, L//2^(4-current_level), Dx]
        even = even.permute(1, 0, 2)  # [L//2^(4-current_level), B, Dx]
        odd = odd.permute(1, 0, 2)  # [L//2^(4-current_level), B, Dx]
        ...
        return torch.cat(zipped_data, 0).permute(1, 0, 2)  # [B, L//2^(4-current_level+1), Dx]
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # position-encoding
        pe = self.get_position_encoding(x_enc)  # [1, L, Dx']
        ...
        # SCINet
        dec_out = self.sci_net_1(x_enc)  # [B, L, Dx]
        dec_out += x_enc
        dec_out = self.projection_1(dec_out)  # [B, P, Dx]
        if self.num_stacks != 1:
            dec_out = torch.cat((x_enc, dec_out), dim=1)  # [B, L+P, Dx]
            temp = dec_out
            dec_out = self.sci_net_2(dec_out)  # [B, L+P, Dx]
            dec_out += temp
            dec_out = self.projection_2(dec_out)  # [B, L+P, Dx]
    ...
    def get_position_encoding(self, x):
        max_length = x.size()[1]  # L
        position = torch.arange(max_length, dtype=torch.float32, device=x.device)  # [L]
        scaled_time = position.unsqueeze(1) * self.inv_timescales.unsqueeze(0)  # [L, 1] * [1, Dx'//2] -> [L, Dx'//2]
        signal = torch.cat([torch.sin(scaled_time), torch.cos(scaled_time)], dim=1)  # [L, Dx']
        signal = F.pad(signal, (0, 0, 0, self.pe_hidden_size % 2))  # [L, Dx']
        signal = signal.view(1, max_length, self.pe_hidden_size)  # [1, L, Dx']
